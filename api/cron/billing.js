// CRON (daily 08:00) — Monthly recurring billing (spec 3.1 / 3.4 #41).
// Charges the stored Paystack authorization for full memberships whose
// next_billing_date has arrived. Advances the billing date on attempt so a
// period is never double-charged; failures are handled by the retry cron.
...
import { onPaymentReceived, notifyOwner } from '../_lib/notify/index.js';
...
export default async function handler(req, res) {
  ...
  if (!paystackConfigured()) return ok(res, { skipped: 'paystack_not_configured' });
  ...
  try {
    ...
    for (const m of due || []) {
      ...
      // Always move the next billing date forward so we don't re-pick this period.
      await supabase
        .from('memberships')
      ...
      try {
        ...
        if (data.status === 'success') {
          ...
          throw new Error(data.gateway_response || 'charge_declined');
        }
      } catch (err) {
        await supabase
          .from('payments')
    ...
    return ok(res, { processed: (due || []).length, charged, failed });
  } catch (err) {
