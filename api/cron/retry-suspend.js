// CRON (hourly) — Failed payment retry + suspension (spec 3.1 / 3.4 #42).
// Retries failed payments whose retry time has arrived. After a second failure
// the member is suspended (access blocked at verification) and everyone notified.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return ok(res, { processed: (failures || []).length, recovered, suspended });
  } catch (err) {
