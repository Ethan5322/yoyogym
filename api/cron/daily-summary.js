// CRON (daily 20:00) — Owner daily summary (spec Part 5 #10 / 3.4 #45).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await notifyOwner(
      supabase,
      'daily_summary',
      ownerTemplates.daily_summary({ checkins, revenue, newMembers, classesHeld })
    );

    return ok(res, { checkins, revenue, newMembers, classesHeld });
  } catch (err) {
