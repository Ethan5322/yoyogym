...
export async function generateMembershipPdf(data) {
  ...
  // Robust download (works where doc.save() is silently blocked). Any failure
  // propagates so the success screen can tell the member what happened.
  return downloadPdf(doc, `${member.membership_number}.pdf`);
}
